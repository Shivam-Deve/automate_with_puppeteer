...
launchBrowser.then(function (browserInstance) {
    ...
    let kitClick = gtab.click(".recommended-prep-kit-card-cta.ui-btn-link", { delay: 100 })
    return kitClick
}).then(() => {
    return gtab.waitForSelector('.interview-ch-li-grid>a');
}).then(function () {
    let questionArr = gtab.$$(".interview-ch-li-grid>a")
    return questionArr
}).then((questionArr) => {
